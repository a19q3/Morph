...
| REC-W6-07 / W6-RIGOR-04 | Corrected after model re-audit | host and script-common `same_context_except_progress` allow participant-signed settlement descriptor evolution, preserve descriptor version and the materialised Vault root, and reserve Vault replacement for the stricter `state_context_matches_splice_next` bridge. |
