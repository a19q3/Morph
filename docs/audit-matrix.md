...
This matrix is read against the current witness-envelope implementation line.
Remaining `*current` fixture and witness names identify bounded body/package
...
| --- | --- |
| One live State Cell controls the channel pointer; initial configuration has bilateral consent and exactly one committed Vault output | `state_type_accepts_canonical_initial_state`, `state_type_rejects_unsigned_initial_state`, `state_type_rejects_initial_state_without_committed_vault`, `state_type_rejects_initial_state_with_ambiguous_committed_vault`, `accepts_valid_state_supersession`, `rejects_stale_or_equal_state_number` |
...
| Factory creation and full state evidence are signed by all current-profile participants | `factory_type_accepts_canonical_initial_factory_state`, `factory_type_rejects_unsigned_initial_factory_state`, `factory_type_accepts_signed_factory_update`, `factory_type_rejects_invalid_participant_signature` |
...
| Signed StateHeader progress may update settlement payouts while preserving the materialised Vault, and final outputs match the signed current descriptor | `accepts_signed_settlement_descriptor_update`, `rejects_unsigned_settlement_descriptor_update`, `rejects_signed_vault_materialisation_update_as_context_change`, `state_type_accepts_signed_descriptor_update`, `state_type_rejects_signed_materialisation_update`, `vault_lock_accepts_finalise_with_current_state`, `vault_lock_rejects_descriptor_output_mismatch` |
