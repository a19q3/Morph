...
| Operation taxonomy | Core has the current executable set: `Fund`, `Publish`, `Supersede`, `Finalise`, `Splice`, and `Materialise`. Contracts represent `FUND` as State type creation rather than a witness-carried operation tag. | Paper defines the wider profile set `ChannelOperation = {FUND, PUBLISH, SUPERSEDE, FINALIZE, COOPERATIVE_CLOSE, SPLICE, MATERIALIZE}` and marks cooperative close as future-profile work. | Aligned as a current-profile subset; the contract-level FUND shape and absent cooperative-close branch are documented profile details. |
...
| FUND authority | Scripts verify canonical anchor derivation, active phase, state number zero, lock/type binding, capacity, and a bilateral signature witness over the complete initial StateHeader. Initial Factory creation likewise requires the full Factory signature envelope. | The paper's strict initial authority profile is implemented; splice-created successors remain authorised by the signed splice bridge. | Aligned and executable. |
...
| COOPERATIVE_CLOSE | Not present in the current core operation enum, host-side vault operation switch, State type, vault contract, CLI, or devnet flow. | Paper marks cooperative close as a protocol branch and future deployment profile, not current executable evidence. | Aligned as a documented profile restriction. Implementation work remains if cooperative close enters the milestone. |
...
3. Decide whether cooperative close is in scope for a future implementation
   milestone. Today it is explicitly absent from the core operation enum and is
   documented as a future deployment profile, not as a script or devnet flow.
