...
| W5-04 | Fixed | `same_context_except_progress` now preserves settlement descriptor commitment and descriptor version in host and script-common, while still allowing profile-specific payload changes. |
...
| W5-14 | Fixed for the active envelope set | Factory envelope kinds are routed by `WitnessEnvelope::parse` and script-specific witness parsers; contract tests exercise the active CKB-VM routes. |
| W5-15 | Fixed | `witness_envelope_accepts_every_known_kind_and_rejects_bad_body_lengths` directly covers every known envelope kind and rejects invalid body lengths at the envelope layer. |
