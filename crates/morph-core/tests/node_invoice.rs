...
fn invoice_round_trips_and_rejects_tampering() {
    let invoice = MorphInvoice::new(invoice_request(bytes32(9))).unwrap();
...
fn invoice_settlement_requires_matching_preimage_and_live_invoice() {
    ...
    let stored = node.create_invoice(invoice_request(preimage)).unwrap();
    ...
    let mut expired = MorphNodeState::new(bytes32(3), MorphNetwork::Devnet).unwrap();
    let expired_invoice = expired
        .create_invoice(invoice_request(bytes32(13)))
        .unwrap();
    assert_eq!(
        expired
