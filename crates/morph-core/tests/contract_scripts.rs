...
use morph_script_common::{
    ...
    CONDITIONAL_MIN_PLAIN_OUTPUT_CAPACITY, ECDSA_SIGNATURE_LEN, FACTORY_COMPACT_PROOF_LEN,
    FACTORY_COMPACT_PROOF_MAX_SIBLINGS, FACTORY_COMPACT_PROOF_PAIR_LEN,
    FACTORY_LOCAL_EXIT_WITNESS_VERSION, FACTORY_MERKLE_UPDATE_RIGHT_COUNT,
    FACTORY_MERKLE_UPDATE_WITNESS_VERSION, FACTORY_MIN_PARTICIPANTS,
    FACTORY_MULTI_RIGHT_UPDATE_WITNESS_VERSION, FACTORY_REDUCED_EXIT_RIGHTS_COUNT,
    ...
    FACTORY_REDUCED_RIGHTS_WITNESS_VERSION, FACTORY_REDUCED_SPLICE_WITNESS_VERSION,
    FACTORY_RIGHT_KIND_RESERVE_CLAIM, FACTORY_RIGHT_LEN, FACTORY_SIGNATURE_WITNESS_VERSION,
    FACTORY_SPARSE_MERKLE_DEPTH, FACTORY_SPLICE_HEADER_LEN, FACTORY_SPLICE_WITNESS_VERSION,
    FACTORY_STATE_HEADER_LEN, FACTORY_VAULT_ASSET_AMOUNT_LEN, FACTORY_VAULT_DELTA_LEN,
    FACTORY_VAULT_DELTAS_LEN, FACTORY_VAULT_DESCRIPTOR_LEN, FactoryMerkleUpdateWitness,
    FactoryMultiRightUpdateWitness, FactoryReducedExitWitness, FactoryReducedRightsWitness,
...
fn conditional_batch_resolution_tx(tamper: ConditionalBatchTamper) -> (Context, TransactionView) {
    ...
    let mut descriptor = batch.encode_descriptor().unwrap();
    if tamper == ConditionalBatchTamper::DustParticipantOutput {
        put_u64(
            &mut descriptor,
            4 + 2 * BYTE32_LEN + 1 + 7 + BYTE32_LEN,
            CONDITIONAL_MIN_PLAIN_OUTPUT_CAPACITY - 1,
        );
    }
    let mut witness = batch.encode_resolution_witness(&resolutions).unwrap();
    if tamper == ConditionalBatchTamper::DustParticipantOutput {
        witness[4..4 + descriptor.len()].copy_from_slice(&descriptor);
    }
...
#[ignore = "requires `make build-contracts`"]
#[test]
fn batch_lock_rejects_dust_participant_output_descriptor() {
    let (context, tx) =
        conditional_batch_resolution_tx(ConditionalBatchTamper::DustParticipantOutput);
    assert!(context.verify_tx(&tx, MAX_CYCLES).is_err());
}
