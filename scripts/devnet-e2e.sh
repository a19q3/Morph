...
E2E_DIR="${E2E_DIR:-target/devnet-e2e/$RUN_ID}"
CKB_DIR="${CKB_DIR:-$E2E_DIR/node}"
OUT_DIR="${OUT_DIR:-$E2E_DIR/smoke}"
LOG_DIR="$E2E_DIR/logs"
LATEST_LINK="${LATEST_LINK:-target/devnet-e2e/latest}"
...
log "running on-chain devnet smoke without local cargo/testtool checks"
...
MORPH_DEVNET_SMOKE_SKIP_LOCAL_CHECKS=1 \
  scripts/devnet-smoke.sh 2>&1 | tee "$LOG_DIR/devnet-smoke.log"

log "running budget-backed smoke assertion"
...
log "passed; artefacts are in $E2E_DIR"
